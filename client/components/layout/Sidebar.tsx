...
export function Sidebar({
    ...
    <Link
      ...
      className={cn(
        ...
        collapsed
          ...
          : "gap-3 h-10 w-full px-3 justify-start",
  ...
  return (
    <aside
      className={cn(
        "fixed left-0 top-0 h-screen flex flex-col justify-between py-4 bg-gradient-to-b from-indigo-900 to-slate-900 border-r border-white/10",
        collapsed ? "items-center w-20" : "items-stretch w-64",
      )}
    >
      <div
        className={cn(
      ...
      >
        ...
        {user.role === "admin" && (
          <Item to="/distributor" icon={Network} label="Distributor" />
        )}
      ...
      <div className={cn("relative", collapsed ? "px-0" : "px-3")}>
        ...
        <button
          ...
          className={cn(
            "flex items-center rounded-full text-red-400 hover:text-red-300 hover:bg-white/5",
            collapsed
