  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        ...
        const ucount = inbox.filter((m: any) => !m.readBy.includes((user as any)?.id)).length;
        setUnread(ucount);
      } catch {}
    }
    if (user) {
      ...
      <header className="sticky top-0 z-40 backdrop-blur supports-[backdrop-filter]:bg-white/5 bg-white/5 border-b border-white/10">
        <div className="container mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm">
            ...
            {user?.role === "admin" && <>
              <NavLink to="/team" label="Team" current={loc.pathname.startsWith("/team")} />
              <NavLink to="/distributor" label="Distributor" current={loc.pathname.startsWith("/distributor")} />
              <NavLink to="/jobs" label="Jobs" current={loc.pathname.startsWith("/jobs")} />
            </>}
            {user && <NavLink to="/chat" label="Chat" current={loc.pathname.startsWith("/chat")} />}
            {user && <NavLink to="/inbox" label={`Inbox${unread ? ` (${unread})` : ""}`} current={loc.pathname.startsWith("/inbox")} />}
          </nav>
          <div className="flex items-center gap-2">
            {!user ? (
              ...
              <>
                <span className="hidden sm:block text-white/70 text-sm mr-2">{user.name} ({user.role})</span>
                <Button variant="secondary" onClick={() => navigate(user.role === "admin" ? "/distributor" : "/inbox")}>Dashboard</Button>
                <Button variant="ghost" className="text-white/80" onClick={() => { logout(); navigate("/"); }}>Logout</Button>
              </>
            )}
          </div>
        </div>
...
function NavLink({ to, label, current }: { to: string; label: string; current: boolean }) {
  return (
    <Link to={to} className={cn("hover:text-white/90 transition", current ? "text-white" : "text-white/70")}>{label}</Link>
  );
}
