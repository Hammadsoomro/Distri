...
export default function Distributor() {
  ...
  const start = async () => {
    ...
    try {
      ...
      const res = await DistributorApi.createJob({ text, intervalSec, linesPerTick, targetIds });
      setJob(res.job);
      setText("");
    } catch (e: any) {
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            <div className="grid sm:grid-cols-2 gap-2">
              {members.map((m) => (
                <label key={m.id} className="flex items-center gap-2 p-2 rounded bg-white/5 border border-white/10">
          ...
          <Button type="button" onClick={start} disabled={!text.trim() || !Object.values(selected).some(Boolean)}>Start</Button>
        </CardContent>
      </Card>
      ...
      <Card className="bg-white/5 border-white/10 text-white">
        ...
        <CardContent>
          <div className="space-y-3">
            {jobs.map((j) => (
              <div key={j.id} className="p-3 rounded bg-white/5 border border-white/10 flex items-center justify-between">
                <div>
                  <p className="font-medium">{j.status.toUpperCase()} • {j.linesPerTick} lines • {j.intervalSec}s</p>
                  <p className="text-white/60 text-sm">Sent {j.nextIndex}/{j.textLines.length} lines</p>
                </div>
                {j.status === "running" ? (
                  <Button variant="destructive" onClick={() => cancel(j.id)}>Cancel</Button>
                ) : (
