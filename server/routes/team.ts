...
export const createMember: RequestHandler = (req, res) => {
  ...
  for (const u of db.users.values()) if (u.email.toLowerCase() === email.toLowerCase()) return res.status(400).json({ error: "Email already in use" });
  const id = newId("user");
  const user = { id, name, email, passwordHash: hashPassword(password), role: "member" as const, inbox: [] };
...
export const deleteMember: RequestHandler = (req, res) => {
  ...
  if (!user || user.role !== "member") return res.status(404).json({ error: "Not found" });
  db.users.delete(id);
  res.json({ ok: true });
};
